...
* 가장 중요한 이진 힙의 수행시간
insert 연산을 위한 upheap은 삽입된 노드로부터 최대 루트까지 올라가며 부모와 자식노드를 교환다.
또한 Delete_min 연산에서는 힙의 마지막 노드를 루트로 이동한 후, downheap을 최하위층의 노드까지 교환해야하는 경우가 발생한다.
따라서 힙에서 각 연산의 수행시간은 힙의 높이에 비례한다. 그런데 힙은 완전이진트리이므로 힙에 n개의 노드가 있으면 그 높이는 log(n+1)이다.
따라서 각 힙의 연산의 수행시간은 o(logN)이다. 
그러나 힙 생성에 있어서는 h=1인 경우부터 시작해 최상위층 루트까지 각 노드에 대입해 downheap을 수행하므로, o(n)의 수행시간을 갖는다.


++ python 에서의 힙 모듈 - heapq
import heapq를 이용해 힙 모듈을 임포트 할 수 있다. 그리고 이것은 자바에서의 PQ처럼 리스트와 별개인 클래스가 아니라 리스트를 최소힙처럼 다룰 수 있게 도와주는 것이다.
